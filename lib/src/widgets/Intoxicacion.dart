...
// Ya no se usa SvgPicture.string ni ui.ImageFilter
// import 'package:flutter_svg/flutter_svg.dart';
// import 'dart:ui' as ui;

class Intoxicacion extends StatefulWidget {
  ...
  @override
  _IntoxicacionState createState() => _IntoxicacionState();
}

class _IntoxicacionState extends State<Intoxicacion> {
  final TextEditingController _sustanciaIngeridaController = TextEditingController(); // Para "SUSTANCIA INGERIDA"
  final TextEditingController _comportamientoSintomasController = TextEditingController();
  ...
  @override
  void dispose() {
    _sustanciaIngeridaController.dispose();
    _comportamientoSintomasController.dispose();
    super.dispose();
  }
    ...
    final Animal? animalEscogido = await Navigator.push<Animal?>(
      ...
      MaterialPageRoute(
        builder: (context) => const ListadeAnimales(
          key: Key('seleccionAnimalParaIntoxicacion'),
  ...
  Future<void> _guardarIntoxicacion() async {
    ...
    if (_sustanciaIngeridaController.text.trim().isEmpty ||
        _comportamientoSintomasController.text.trim().isEmpty) {
      _mostrarSnackbar(
          "Por favor, completa todos los campos sobre la intoxicación.", Colors.orange);
      return;
    }

    try {
      final intoxicacionData = {
        'sustanciaIngerida': _sustanciaIngeridaController.text.trim(),
        'comportamientoSintomas': _comportamientoSintomasController.text.trim(),
      ...
      await FirebaseFirestore.instance
          ...
          .collection('registros_intoxicaciones') // Nueva subcolección
          .add(intoxicacionData);

      _mostrarSnackbar(
          "Registro de intoxicación para ${_animalSeleccionado!.nombre} guardado con éxito.",
          Colors.green);
      _sustanciaIngeridaController.clear();
      _comportamientoSintomasController.clear();
    } catch (e) {
                      ...
                      child: ClipRRect(
                        ...
                        child: profilePhotoUrl != null && profilePhotoUrl.isNotEmpty
                            ...
                            errorWidget: (context, url, error) {
                              print("Error cargando foto de perfil del usuario (Intoxicacion): $error");
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Row(
                      ...
                      children: [
                        Image.asset('assets/images/intoxicacion.png', width: 45.0, height: 45.0),
                    ...
                    const SizedBox(height: 20.0),
                    const Text( // Nuevo campo para sustancia ingerida
                      'SUSTANCIA INGERIDA (SI SE CONOCE):',
                      style: TextStyle(fontFamily: 'Comic Sans MS', fontSize: 17, color: Color(0xff000000), fontWeight: FontWeight.w700),
                    ),
                    const SizedBox(height: 8.0),
                    TextField(
                      controller: _sustanciaIngeridaController,
                      decoration: InputDecoration(
                        hintText: 'Ej: Chocolate, raticida, planta tóxica...',
                    ...
                    const SizedBox(height: 20.0),
                    const Text(
                      'COMPORTAMIENTO Y SÍNTOMAS OBSERVADOS:',
                      style: TextStyle(fontFamily: 'Comic Sans MS', fontSize: 17, color: Color(0xff000000), fontWeight: FontWeight.w700),
                    ),
                    const SizedBox(height: 8.0),
                    TextField(
                      controller: _comportamientoSintomasController,
                      maxLines: 4,
                      decoration: InputDecoration(
                        hintText: 'Describe el comportamiento anómalo (ej: agitación, letargo) y los síntomas físicos (ej: vómitos, diarrea, temblores, convulsiones, babeo, dificultad para respirar)...',
                        hintStyle: TextStyle(fontFamily: 'Comic Sans MS', fontSize: 15, color: Colors.grey.shade700),
                        filled: true,
                        fillColor: Colors.white.withOpacity(0.9),
                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(10.0), borderSide: const BorderSide(color: Color(0xff707070))),
                    ...
                    Row(
                      ...
                      children: <Widget>[
                        ElevatedButton(
                          ...
                          onPressed: _guardarIntoxicacion,
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(builder: (context) => SolucionAEMERGENCIAS(key: const Key('SolucionAEMERGENCIAS_Intoxicacion'), descripcionDelProblema: '',)),
                            );
                          },
